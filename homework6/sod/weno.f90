module WENO
    implicit none
    save
    integer, parameter::cells = 1000
    real, dimension(3,cells+1)::u                                                  ! 守恒变量
    real, dimension(3, cells+1)::h_flux, h_flux_plus, h_flux_minus
    real, parameter::u1 = 0, rho1 = 1, p1 = 1, u2 = 0, rho2 = 0.125, p2 = 0.1      ! sod激波管问题初值
    ...
    real, parameter::delta_x = 2.0/cells                                       ! 网格点之间的间距
    ...
    subroutine h_flux_calc_plus(f_tar)
        ...
        real, parameter::epsilon=1e-6, para_p=3      ! WENO参数
    ...
    subroutine rungekutta()
        ...
        ! 最终数据存储在该文件中
        open(unit=101, file='weno.dat', status='old', &
            action='write')
